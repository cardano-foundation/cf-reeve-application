...
backend-test:
  ...
  WITH DOCKER \
    ...
    RUN docker run -v /var/run/docker.sock:/var/run/docker.sock --rm --entrypoint=bash backend-test:latest -c "docker ps -a; ./gradlew --full-stacktrace -Dorg.gradle.debug=true clean test"
  END
